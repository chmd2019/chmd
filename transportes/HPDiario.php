...
ob_start ();
$existe = '';
$datos = mysql_query ( "select vpd.id,vpd.fecha2,vs.correo,vpd.calle_numero,vpd.colonia,
       vpd.cp,vpd.ruta,vpd.comentarios,vpd.estatus,vpd.nfamilia,
       ...
       vpd.mensaje,usu.familia,vpd.fecha1,vpd.fecha,usu.telefonomama
from 
Ventana_Permiso_diario vpd
LEFT JOIN Ventana_user vs on vpd.idusuario=vs.id
LEFT JOIN usuarios usu on vpd.nfamilia=usu.`password`
LEFT JOIN alumnoschmd alu1 on vpd.alumno1=alu1.id
LEFT JOIN alumnoschmd alu2 on vpd.alumno2=alu2.id
LEFT JOIN alumnoschmd alu3 on vpd.alumno3=alu3.id
LEFT JOIN alumnoschmd alu4 on vpd.alumno4=alu4.id
LEFT JOIN alumnoschmd alu5 on vpd.alumno5=alu5.id
where vpd.archivado=1 and YEAR(vpd.fecha)=$year and MONTH(vpd.fecha)=$mes order by vpd.id desc" );
if (isset ( $_POST ['nombre_nivel'] )) 
	...
	if ($nombre) {
		...
		if ($status==3) 
                    {
			$query = "UPDATE Ventana_Permiso_diario SET mensaje = '$mensaje',estatus=3 WHERE id=$funcion";
			mysql_query ( $query );
			$json = array (
					'estatus' => '0' 
			);
                    ...
                    {
			$query = "UPDATE Ventana_Permiso_diario SET mensaje = '$mensaje',estatus=2 WHERE id=$funcion";
			mysql_query ( $query );
			$json = array (
					'estatus' => '0' 
			);
	...
	<div class="container">
		...
		<div class="masthead">
			...
			<ul class="nav nav-justified">
				<li class="active"><a href="#">Historial Diario</a></li>
                                <li><a href="HPViaje.php">Historial Temporal</a></li>
                                <li><a href="HPPermanente.php">Historial Permenente</a></li>
			</ul>
		</div>
		<br>
