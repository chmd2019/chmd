...
class Ctr_uniformes
{
 ...
 function actualizar($campos,$correo){
		if($this->con->conectar1()==true){
			//print_r($campos);
			return mysql_query("INSERT INTO pedido_uniforme(talla_sudadera,talla_pants,talla_playera,talla_educacionf,talla_kinder,idalumno,status,nfamilia,fecha,tiposuda,tipoplayera,tipopants,tipoedu,correo) VALUES ('".$campos[0]."', '".$campos[1]."','".$campos[2]."','".$campos[3]."','".$campos[4]."','".$campos[5]."','".$campos[6]."','".$campos[7]."',now(),'".$campos[8]."','".$campos[9]."','".$campos[10]."','".$campos[11]."','".$correo."')");
        ...
        {
		if($this->con->conectar1()==true){
                    return mysql_query("SELECT * FROM uniformes_alumnos where idfamilia="."' $nfamilia'");
		}
        }  
   /////////////////////////proceso de correo adicional//////
        ...
        {
		if($this->con->conectar1()==true){
                    return mysql_query("select id,nfamilia,correo,notificacion from pedido_uniforme_adicional where notificacion=0 and  `status`=3 GROUP BY nfamilia limit 1");
