<a class="waves-effect waves-light modal-trigger" href="#modal_guia<?php echo "$id";   ?>">
...
$url = $link;
 ?>
<div class="modal modal-fixed-footer" id="modal_guia<?php echo "$id"; ?>"  style="width: 95%;height: 90%;max-height: 90%;top: 5%;">
   <div class="modal-dialog">
    <div class="modal-content" style="padding:1px">
      <div class="modal-header" style="padding:5px">
       <h6 class="modal-title text-center" ><?=$nombre?></h6>
     </div>
      <div class="modal-body">
        <div style="text-align: center;">
          <?php
              if (!$encontrado){
                ?>
                <iframe src="https://docs.google.com/gview?url=<?php echo urlencode($url); ?>&embedded=true"
        ...
        <div class="" style="text-align:justify; padding:20px">
          ...
          <p><b>A quien corresponda:</b> Hemos leído cuidadosamente la Guía de servicios y procedimientos administrativos, en las que se encuentran estipuladas las disposiciones y lineamientos  administrativos; así como las cuotas para el ciclo escolar 2019-2020 y quedamos enterados y de acuerdo con su contenido y en que, su exacto cumplimiento será para beneﬁcio de nuestro hijo (a). Asimismo, nos comprometemos a cooperar con el Colegio en la consecución del logro de la labor educativa y, por lo tanto, convenimos en cumplir y hacer cumplir a nuestro (a), hijo (a) los mencionados reglamentos y demás normatividad.
          </p>
          <br>
          ...
          <?php if (!$encontrado){
            ...
            <p>
              <label>
                <input id="autorizo_guia"  name="accion"  type="checkbox" onchange="habilita_boton_guia()"/>
                <span style="color:#000"><b>Acepto términos y condiciones de la Guía de servicios y procedimientos administrativos.</b></span>
              </label>
            </p>
            <?php
          }else {
              ...
              <p>
                <label>
                  <input id="autorizo_guia"  name="accion"  type="checkbox"  checked disabled/>
                  <span style="color:#000"><b>Acepto términos y condiciones de la Guía de servicios y procedimientos administrativos.</b></span>
                </label>
              </p>
              <?php
    ...
    <div class="modal-footer">
      ...
      <a disabled id="boton_guia" href="#!" class="waves-effect btn-flat b-azul white-text <?php if ($encontrado) echo 'd-none'; ?>" onclick="autorizar_guia('<?php echo "$familia"; ?>','<?=$id?>')">Enviar</a>
      <?php if ($encontrado){
        ?>
...
function autorizar_guia(nfamilia, idarchivo) {
  //desabilitar boton
  $('#boton_guia').attr('disabled','disabled');

  var modal_guia = M.Modal.getInstance($("#modal_guia"+idarchivo));
  var estatus = '2';
  $.ajax({
    ...
    success: function (res) {
      if (res == 1) {
        ...
        $('#boton_guia').removeAttr('disabled');
      }
    }
  });
}

function habilita_boton_guia() {
  if ($("#autorizo_guia").prop('checked')){
    $('#boton_guia').removeAttr('disabled');
  }else{
    $('#boton_guia').attr('disabled','disabled');
  }
