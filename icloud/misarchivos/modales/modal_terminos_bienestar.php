<a class="waves-effect waves-light modal-trigger" href="#modal_bienestar<?php echo "$id"; ?>">
...
$url = $link;
 ?>
<div class="modal modal-fixed-footer" id="modal_bienestar<?php echo "$id"; ?>"  style="width: 95%; height: 90%;max-height: 90%; top: 5%;">
   <div class="modal-dialog">
    <div class="modal-content" style="padding:1px">
      <div class="modal-header" style="padding:5px">
       <h6 class="modal-title text-center" >Bienestar CHMD 19-20</h6>
     </div>
      <div class="modal-body">
        <div style="text-align: center;">
          ...
          if (!$encontrado){
            ?>
            <iframe id="ventana_pdf"  src="https://docs.google.com/gview?url=<?php echo urlencode($url); ?>&embedded=true&zoom=false"
              style="width:100%; height:400px" frameborder="0"></iframe>
            <?php
          }
           ?>
        </div>
        <div class="" style="text-align:justify; padding:20px">
          <b>Colegio Hebreo Maguen David, A.C</b><br><br>
          <p><b>A quien corresponda:</b> Hemos leído cuidadosamente el documento #BIENESTARCHMD, en las que se encuentran estipuladas las disposiciones y lineamientos académicos;  quedamos enterados y de acuerdo con su contenido y en que, su exacto cumplimiento será para beneﬁcio de nuestro hijo (a). Asimismo, nos comprometemos a cooperar con el Colegio en la consecución del logro de la labor educativa y, por lo tanto, convenimos en cumplir y hacer cumplir a nuestro (a), hijo (a) los mencionados reglamentos y demás normatividad.
          </p>
          <br>
          ...
          <p>
            <?php
                if(isset($papa)){
                  ?>
                  <b>Nombre del Padre o tutor:</b> <?=$papa?> <br>
                  <?php
                }
                  if(isset($mama)){
                    ?>
                  <b>Nombre de la Madre o tutora:</b> <?=$mama?> <br>
                    <?php
                  }
                  ?>
                  <?php
          ...
          <?php if(!$encontrado){
            ...
            <p>
              <label>
                <input id="autorizo_bienestar"  name="accion"  type="checkbox" onchange="habilita_boton_bienestar()"/>
                <span style="color:#000"><b>Acepto términos y condiciones de #BIENESTARCHMD.</b></span>
              </label>
            </p>
            <?php
          }else{
            ...
            <p>
              <label>
                <input id="autorizo_bienestar"  name="accion"  type="checkbox"  checked disabled/>
                <span style="color:#000"><b>Acepto términos y condiciones de #BIENESTARCHMD.</b></span>
              </label>
            </p>
            <?php
    ...
    <div class="modal-footer">
      ...
      <a disabled id="enviar_bienestar" href="#!" class="waves-effect btn-flat b-azul white-text  <?php if ($encontrado) echo 'd-none'; ?>" onclick="autorizar_bienestar( '<?php echo "$familia"; ?>', '<?=$id?>')" >Enviar</a>
      <?php if ($encontrado){
        ?>
        <a href="<?=$link?>" target="_blank" class="waves-effect btn-flat b-azul white-text" >Descargar</a>
        <?php
      } ?>
    </div>
  </div>
...
<script>
  ...
  $("#ventana_pdf").load(function(){
    $(".ndfHFb-c4YZDc-GSQQnc-LgbsSe.ndfHFb-c4YZDc-to915-LgbsSe.VIpgJd-TzA9Ye-eEGnhe.ndfHFb-c4YZDc-LgbsSe").css("display","none");
  //  $(".ndfHFb-c4YZDc-i5oIFb.ndfHFb-c4YZDc-e1YmVc .ndfHFb-c4YZDc-Wrql6b-AeOLfc-b0t70b .ndfHFb-c4YZDc-GSQQnc-LgbsSe").css("display","none");
  });

    function autorizar_bienestar( nfamilia, idarchivo) {
      $('#enviar_bienestar').attr('disabled','disabled');
      var modal_bienestar = M.Modal.getInstance($("#modal_bienestar"+idarchivo));
      var estatus = '2';
        $.ajax({
            url: "https://www.chmd.edu.mx/pruebascd/icloud/misarchivos/common/post_autoriza_archivo.php",
            type: "POST",
            data: {
              submit: '1',
              nfamilia: nfamilia,
            ...
            success: function (res) {
                if (res == 1) {
                    modal_bienestar.close();
                    M.toast({html: "¡Solicitud realizada con éxito!", classes: 'green accent-4'});
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
                  M.toast({html: "¡Solicitud no realizada!", classes: 'red accent-4'});
                  $('#enviar_bienestar').removeAttr('disabled');
                }
            }
        });
    }

    function habilita_boton_bienestar() {
      if ($("#autorizo_bienestar").prop('checked')){
        $('#enviar_bienestar').removeAttr('disabled');
      }else{
        $('#enviar_bienestar').attr('disabled','disabled');
      }
