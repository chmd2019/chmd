...
$eventos_montaje = $control->lista_ensayos();
$eventos_array = array();

while ($row = mysqli_fetch_array($eventos_montaje)) {
  $titulo =  'Ensayo -' . $row['nombre_evento'];
  $titulo = (strlen($titulo) > 6 ) ?  substr( $titulo, 0, 6).'...' : $titulo;
  array_push($eventos_array, [
    "title" => $titulo,
    "start" => $row['fecha_ensayo_simple']. 'T'. $row['horario_inicial'],
    "end" => $row['fecha_ensayo_simple']. 'T'. $row['horario_final']
  ]);
}
