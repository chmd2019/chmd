    ...
    if ($consulta = mysqli_fetch_array($consulta)) :
        ...
        <div class="row">
            <div class="col s12 l8 b-blanco border-azul" style="float: none;margin: 0 auto;">                
                ...
                <h5 class="center-align c-azul">Solicitud de requerimientos de montaje permanente</h5>
                <br>
                <?php if ($id_privilegio == 3): ?>    
                    <div class="row" style="padding:0rem .5rem;">
                        ...
                        <div class="col s12 l6">
                            <div class="input-field">
                                <i class="material-icons prefix c-azul">calendar_today</i>
                                <input type="text" name="datefilter" autocomplete="off"/>
                        ...
                        <div class="input-field col s12 l6">
                            ...
                            <input 
                                ...
                                onclick="validar_horarios('horario_evento', this.id);"
                                autocomplete="off"
                                onfocus="blur();"
                                placeholder="Seleccione horario">  
                        </div> 
                        ...
                        <div class="input-field col s12" hidden id="caja_cafe">
                            <div class="input-field col s12 l6" id="select_lugar_evento_solo_cafe">
                                ...
                                <select id="lugar_evento_solo_cafe" 
                                        onchange="lugar_evento_solo_cafe = this.value;">
                                </select>
                                <label style="margin-left: 1rem">Lugar del evento</label>
                            </div> 
...
<script>
    ...
    function validar_horarios(id_inicial, id) {
        var hora_inicial = parseInt($("#" + id_inicial).val().split(":")[0]);
        $('#' + id).timepicker('remove');
        var hora_final = hora_inicial >= 23 ? 23 : hora_inicial + 1;
        $('#' + id).timepicker({
            'step': 30,
            'minTime': `${hora_final}:00`,
            'maxTime': '23:59',
            'timeFormat': 'H:i:s'
        });
        $('#' + id).timepicker('show');
...
//        }
        ...
        if (listaFechas.length === 0) {
            M.toast({
                html: '¡Debe seleccionar un rango de fechas válido para continuar!',
                classes: 'deep-orange c-blanco'
            });
            return;
        }
        ...
        $.ajax({
            url: 'https://www.chmd.edu.mx/pruebascd/icloud/Evento/common/post_nuevo_montaje_permanente.php',
            type: 'POST',
            dataType: 'json',
            beforeSend: () => {
                $("#loading").fadeIn("slow");
    ...
    function setearRango(inicial, final) {
        listaFechas = [];
        moment.locale('es');
        var dia_actual = moment(inicial.format('YYYY-MM-DD'));
        while (dia_actual.isSameOrBefore(final.format('YYYY-MM-DD'))) {
            var dia = dia_actual.format('dddd, DD [de] MMMM [de] YYYY');
            dia = `${dia.charAt(0).toUpperCase()}${dia.slice(1)}`;
            listaFechas.push({
                fecha_simple: dia_actual.format('YYYY-MM-DD'),
                fecha: dia
            });
