...
$datos = mysqli_query ( $conexion,"SELECT vpa.id as idpermisoalumno, vpa.id_alumno,ac.idcursar, ac.nombre, vp.id_permiso,vp.fecha_creacion,
  ...
  where vpa.estatus=2 and vp.tipo_permiso='6' and LOCATE('$year',vp.fecha_cambio )>0  and LOCATE( '$dia', vp.fecha_cambio )>0 and LOCATE( '$mes_letras', vp.fecha_cambio )>0 order by vpa.estatus_seguridad" );
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
   <div class="wrapper">
    ...
    <nav class="" id="sidebar">
      ...
      <?php $perfil_actual='31'; include ('../menus_dinamicos/perfiles_dinamicos_seguridad.php'); ?>
...
<h2 class="text-center text-primary">Seguridad de Internos</h2>
...
<table class="table" id="niveles_table">
  ...
  <tbody class="searchable" style="overflow: auto; max-height: 500px;">
    <?php while ( $dato = mysqli_fetch_assoc ( $datos ) )
    {
      ...
      /* Hora Actual Test
        $hora_actual = strtotime('19-09-18 09:01:00');
      */
      ...
      <tr class="fila_alumnos" id="<?=$idpermiso_alumno?>"  style="background:<?=$color?>; border-bottom:  1px solid <?=$borde?>"  data-row="<?php echo $dato['id_permiso']?>">
        ...
        <td id="img_<?=$idpermiso_alumno?>" onclick="set_estatus(<?=$idpermiso_alumno?>);"  style="text-align:center">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/malihu-custom-scrollbar-plugin/3.1.5/jquery.mCustomScrollbar.concat.min.js"></script>
  ...
  <script type="text/javascript" src="js/PseguridadInternos.js"></script>
