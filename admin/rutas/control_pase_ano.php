...
if (isset($_POST['id_ruta'])){
  ...
  header('Location: PpaseAno.php');
}

$datos = mysqli_query ( $conexion, "SELECT r.nombre_ruta , r.camion , u.nombre, r.auxiliar , r.cupos FROM rutas r LEFT JOIN usuarios u ON u.id=r.auxiliar WHERE r.id_ruta='$id_ruta'  LIMIT 1");
while ($r = mysqli_fetch_assoc($datos) ){
  ...
  $cupos = $r['cupos'] ;
    ...
    <head>
      ...
      <title>CHMD ::  Control de pase año</title>
      ...
      <form id="eventos"  name="eventos" class="form-signin save-nivel"   >
          ...
          <tr>
            <td WIDTH="100%" colspan="3">
              <h4 class="">
                ...
                //mañana
                $n_alumnos_m=0;
                $sql ="SELECT COUNT(*) FROM rutas_pase_ano_alumnos WHERE  id_ruta_base_m='$id_ruta';";
                ...
                //Tarde
                $n_alumnos_t=0;
                $sql ="SELECT COUNT(*) FROM rutas_pase_ano_alumnos WHERE  id_ruta_base_t='$id_ruta';";
                $query = mysqli_query($conexion, $sql);
                if($r = mysqli_fetch_array($query)){
                  $n_alumnos_t= $r[0];
                }
                ...
                <?php include 'componentes/buscador_alumnos_m_pase.php'; ?>
                 <?php include 'componentes/buscador_alumnos_t_pase.php'; ?>
              </h4><br>
            </td>
          </tr>
        </table>
        ...
        <table class="table view_m"  class="text-center"  WIDTH="100%" >
          ...
          <tbody id="lista_alumnos_m">
            ...
            <?php
            $c=-1;
            $sql ="SELECT rb.id_alumno, rb.domicilio_m, rb.hora_manana, ac.nombre, ac.grupo, ac.grado, rb.orden_in FROM rutas_pase_ano_alumnos rb LEFT JOIN alumnoschmd ac ON ac.id = rb.id_alumno WHERE  rb.id_ruta_base_m='$id_ruta' ORDER BY rb.orden_in;";
            $query = mysqli_query ($conexion, $sql);
          ...
          <tbody id="lista_alumnos_t">
            ...
            <?php
            $c=-1;
            $sql ="SELECT rb.id_alumno, rb.domicilio_t, rb.hora_lu_ju, rb.hora_vie, ac.nombre, ac.grupo, ac.grado, rb.orden_out  FROM rutas_pase_ano_alumnos rb LEFT JOIN alumnoschmd ac ON ac.id = rb.id_alumno WHERE  rb.id_ruta_base_t='$id_ruta' ORDER BY rb.orden_out;";
            $query = mysqli_query ($conexion, $sql);
            while ($row = mysqli_fetch_assoc($query)){
              $id_alumno = $row['id_alumno'];
              $nombre = $row['nombre'];
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/malihu-custom-scrollbar-plugin/3.1.5/jquery.mCustomScrollbar.concat.min.js"></script>
  <script type="text/javascript" src="js/control_pase_ano.js"></script>
