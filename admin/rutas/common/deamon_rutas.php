...
if ($conexion) {
      ...
      //copiar base de datos base
        ...
        if ($dia == '5'){
          //es viernes
           $sql="INSERT INTO rutas_historica_alumnos (id_alumno, id_ruta_h,  domicilio, hora_manana, hora_regreso, orden_in, orden_out, fecha, estatus , domicilio_s, id_ruta_h_s)
                                              SELECT id_alumno, id_ruta_base_m, domicilio_m, hora_manana, hora_vie, orden_in, orden_out, '$fecha' , '0' , domicilio_t, id_ruta_base_t FROM rutas_base_alumnos";
        }else{

             $sql="INSERT INTO rutas_historica_alumnos (id_alumno, id_ruta_h,  domicilio, hora_manana, hora_regreso,  orden_in, orden_out, fecha, estatus , domicilio_s, id_ruta_h_s )
                                                  SELECT id_alumno, id_ruta_base_m, domicilio_m, hora_manana, hora_lu_ju, orden_in, orden_out,  '$fecha', '0'  , domicilio_t, id_ruta_base_t FROM rutas_base_alumnos";
        }
        $query = mysqli_query($conexion, $sql);
        if(!$query){
          $error=true;
      ...
      //Leer y modificar Domicilio,ruta y camion por los Cambios Permanentes del dia
        ...
        $sql='COMMIT';
        $dia=1; //TEST PERMANENTE
        switch ($dia) {
          case '1': //lunes
          $sql= "SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_inicial
          FROM Ventana_permisos_alumnos vpa INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha='$fecha' WHERE vp.lunes='lunes' and vp.estatus=2 and vp.tipo_permiso=3;";
            break;
          case '2': //martes
          $sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_inicial
          FROM Ventana_permisos_alumnos vpa INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha='$fecha' WHERE vp.martes='martes' and vp.estatus=2 and vp.tipo_permiso=3 ;";
            break;
          case '3': //miercoles
          $sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_inicial
          FROM Ventana_permisos_alumnos vpa INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha='$fecha' WHERE vp.miercoles='miercoles' and vp.estatus=2 and vp.tipo_permiso=3 ;";
            break;
          case '4': //jueves
          $sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_inicial
           FROM Ventana_permisos_alumnos vpa INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha='$fecha' WHERE vp.jueves='jueves' and vp.estatus=2 and vp.tipo_permiso=3 ;";
            break;
          case '5': //viernes
          $sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_inicial
           FROM Ventana_permisos_alumnos vpa INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha='$fecha' WHERE vp.viernes='viernes' and vp.estatus=2 and vp.tipo_permiso=3 ;";
            break;
        }
        ...
        while($r = mysqli_fetch_array($query)){
          ...
          //verifico que la fecha inicial ya paso
...
// echo $_fecha_inicial;
      ...
      //Leer y modificar Domicilio, ruta, y camion por los Cambios Temporales del dia
        ...
        $sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion,  vp.fecha_inicial, vp.fecha_final, vp.turno
...
// echo $_fecha_final;
		...
		// //(2) Identifico los permisos que Aplican para la fecha
      	  if ($fecha_today>$fecha_inicial and $fecha_today<$fecha_final){
      	   ...
      	   $id_alumno =$r['id_alumno'];
	       $id_ruta = $r['id_ruta'];
	       //decodifico id_ruta, que es id_camion en el permiso
	       $array_ruta = explode('/', $id_ruta);
	       $id_ruta= $array_ruta[0];
	       $id_ruta_s= $array_ruta[1];
          ...
          if ($r=mysqli_fetch_array($query_estatus)){
	        ...
	        $sql_update = "UPDATE rutas_historica_alumnos SET id_ruta_h='$id_ruta', orden_in='$orden', domicilio='$domicilio', estatus='2'  WHERE id_alumno='$id_alumno' and fecha='$fecha'" ;
 			 ...
 			 $sql_update = "UPDATE rutas_historica_alumnos SET id_ruta_h='$id_ruta', orden_in='$orden', domicilio='$domicilio', id_ruta_h_s='$id_ruta_s', orden_out='$orden', domicilio_s='$domicilio', estatus='2'  WHERE id_alumno='$id_alumno' and fecha='$fecha'" ;
			}
	...
	$mes_letras = $arrayMeses[(int)$mes - 1];
		//SQL
	$sql="SELECT vpa.id_alumno, vp.colonia, vp.calle_numero, vp.id_camion as id_ruta, r.nombre_ruta, r.auxiliar, r.camion, vp.fecha_cambio
			FROM Ventana_permisos_alumnos vpa
			INNER JOIN Ventana_Permisos vp ON vp.id_permiso=vpa.id_permiso
			INNER JOIN rutas_historica r ON vp.id_camion=r.id_ruta_h and r.fecha = '$fecha'
	...
	while($r=mysqli_fetch_array($query)){
         ...
         $fecha_cambio = $r['fecha_cambio'];
	       $orden='997';
         //verificar que hoy es el dia de la fecha de cambio.
         //normalizo la fecha
           $array1 = explode(',' , $fecha_cambio ); $array2= explode(' ',$array1[1]);
           $dia= $array2[1]; $mes= '';
           $_mes= strtolower($array2[2]);
           switch ($_mes) {
                 case 'enero':
             ...
             $_fecha_cambio =   $dia.'/'. $mes.'/'.$anio;
             if ($_fecha_today == $_fecha_cambio){
               //es Hoy
               //verifico su estatus
               $sql_estatus="SELECT estatus FROM rutas_historica_alumnos WHERE id_alumno='$id_alumno' and fecha ='$fecha' LIMIT 1";
               $query_estatus = mysqli_query($conexion, $sql_estatus);
