...
<button type="button" style="cursor: pointer; margin:2px; " class="btn btn-primary btn-default pull-right btn-nuevo view_m" data-toggle="modal" data-target="#listModal_m">
<b><span class="glyphicon glyphicon-list-alt"></span>&nbsp;&nbsp;LISTA</b>
</button>
<!---- Modal --->
<div id="listModal_m" class="modal" tabindex="-1" role="dialog" aria-labelledby="ModalCenterTitle" aria-hidden="true" >
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:60vh ; overflow-y: scroll;">
        <table class="table" width="100%"  style="font-size: 14px;"  >
          ...
          <tbody id="lista_alumnos_new_m" class="searchable" >
            <?php
            $sql =  "SELECT DISTINCT ac.id,ac.nombre, ac.grado, ac.grupo, usu.colonia, usu.calle
                    ...
                    WHERE ac.id NOT IN (
                      SELECT id_alumno FROM rutas_generales_alumnos  WHERE id_ruta_general>0
                    ) AND (usu.responsable='PADRE' or usu.responsable='MADRE') AND (ac.id_nivel=1)
                    ORDER BY ac.id";
            ...
            while($alumno = mysqli_fetch_array($existe)){
              ...
              ?>
              <tr style="border-bottom: 1px solid #eee" id="tr_modal_m<?=$id_alumno?>">
                <td id="nombre_modal_m<?=$id_alumno?>"><?=$nombre?></td>
                <td id="domicilio_modal_m<?=$id_alumno?>"><?=$domicilio?></td>
                <td id="grado_modal_m<?=$id_alumno?>"><?=$grado?></td>
                <td id="grupo_modal_m<?=$id_alumno?>"><?=$grupo?></td>
                <td style="text-align:center">
                  <button type="button" class="btn btn-primary" onclick="enlistar_alumno_m(<?=$id_alumno?>)" style ="font-family: 'Varela Round'" >
