...
// }
  ...
  <?php include '../components/header.php'; ?>
    ...
    <center>
      <h2 class="text-primary">Nuevo Control de Ruta</h2>
    </center>
    <?php
      ...
      <form id="eventos"  name="eventos" class="form-signin save-nivel"   >
        <div class="modal-body">
          <table border="0" WIDTH="100%" >
            <tr class="row">
              <select id="ruta_selected" class="form-control" onchange="cupos_disponibles_actual()" name="ruta_selected">
                <option value="0" selected disabled>SELECCIONE UNA RUTA</option>
                <?php
                $datos = mysqli_query ( $conexion,"SELECT * FROM rutas r order by camion;" );
                while($dato =mysqli_fetch_array($datos)){
                  ...
                  $show=true;
                  if ($cupos_disponibles_m>0) $show = false;
                  if ($cupos_disponibles_t>0) $show = false;
                  // if ($camion<1) $show = false;
                  if ($camion>99) $show = false;
                  if($show){
                    ?>
                    <option value="<?=$id_ruta?>" data-cupos="<?=$cupos?>" >RUTA: <?=$nombre_ruta?> | CAMION: <?=$camion?> | CUPOS: <?=$cupos?></option>
                    <?php
                    }
                  }
                 ?>
              </select>
              <input id="cupos_actual" type="hidden" name="cupos_actual" value="">
            </tr>
          </table>
          <br>
        <table border="0" WIDTH="100%">
          <tr>
            <td WIDTH="100%" colspan="3">
              <h4 class="">
                ...
                <b>Listado de Alumnos registrados <span class="view_m">MAÑANA: </span><span id="n_alumnos_m" class="view_m"><?=$n_alumnos_m?></span><span class="view_t" style="display:none" >TARDE: </span><span id="n_alumnos_t" class="view_t" style="display:none" ><?=$n_alumnos_t?></span></b>
        ...
        <div class="modal-footer" style="border:none">
          ...
          <button type="button" class="btn btn-primary" onclick="enviar_formulario()"><b>GUARDAR</b></button>
        </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/malihu-custom-scrollbar-plugin/3.1.5/jquery.mCustomScrollbar.concat.min.js"></script>
  <script type="text/javascript" src="js/nuevo_control_ruta.js"></script>
