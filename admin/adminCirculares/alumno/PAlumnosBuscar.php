...
$total_pages_sql = "SELECT COUNT(*) FROM alumnoschmd WHERE nombre LIKE '%$nb%'";
$result = mysqli_query($conexion,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);
$sql = "SELECT * from alumnoschmd WHERE nombre LIKE '%$nb%' order by idcursar desc, nombre  LIMIT $offset, $no_of_records_per_page";
  ...
  <div class="reload"></div>
    <div class="container" id='principal'>
      ...
      <div class="input-group">
        <form method="POST" action="PAlumnosBuscar.php">
    ...
    <br>
                <a class="btn btn-info btn-xs" href="PAlumnos.php">
                <span class="glyphicon glyphicon-backward"></a></td>
    <br><br>
    ...
    <td colspan="6">
    <ul class="pagination">
    <li style="padding-left:5em"><a href="?pageno=1&nb=$nb">Primera</a></li>
    
    <li style="padding-left:5em" class="<?php if($pageno <= 1){ echo 'disabled'; } ?>">
        <a href="<?php if($pageno <= 1){ echo '#'; } else { echo "?pageno=".($pageno - 1)."&nb=".$nb; } ?>">Anterior</a>
    </li>
    <li style="padding-left:5em" class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>">
        <a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo "?pageno=".($pageno + 1)."&nb=".$nb; } ?>">Siguiente</a>
    </li>
    <li style="padding-left:5em"><a href="?pageno=<?php echo $total_pages."&nb=".$nb;  ?>">Última</a></li>
</ul>
    </td>
    <tr>    
    </tbody>
