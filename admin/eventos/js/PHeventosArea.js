$(function() {
  ...
  $(".fila_alumnos").each(function(){
    //consulto si existen alumnos dentro de ese permiso de ese nivel
    var id_permiso = $(this).attr("data-row");
    //alert(id_permiso+ '-'+ nivel)
    $.get("common/get_alumnos_por_nivel.php", {id_permiso:id_permiso, nivel: nivel }, cantidad_alumnos, 'text' );
    function cantidad_alumnos(res){
      //alert(res);
      var r = JSON.parse(res);
      if(r.nalumnos>0){
          $("#fila_"+ id_permiso).show();
      }else{
        //remover fila
        $("#fila_"+ id_permiso).remove();
      }
    }
  });
  ...
  .submit(
    function(e) {
      ...
      .done(
        function(res) {
          data = JSON.parse(res);
        //  alert('Pendientes:' + data.pendientes + ' estatus: ' + data.estatus );
          if (data.estatus == '-1') {
            alert('Ha ocudrrido un error, Debe autorizar o declinar a todos los alumnos enlistados.');
            } else {
        ...
        function editarNivel(qwert,nfamilia,fecha,correo,estatus, codigo, familia, fecha_evento, tipo_evento, empresa,  motivo, mensaje, responsable, parentesco, fecha_cambio, frespuesta, nivel_perfil)
        {
