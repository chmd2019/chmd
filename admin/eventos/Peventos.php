...
if (!in_array('18', $capacidades)){
  if (in_array('15', $capacidades)) {
    header('Location: PeventosKinder.php');
    exit();
  }
  if (in_array('16', $capacidades)) {
    header('Location: PeventosPrimaria.php');
    exit();
  }
  if (in_array('17', $capacidades)) {
    header('Location: PeventosBachillerato.php');
    exit();
  }
    ...
    <nav class="" id="sidebar">
      <div id="dismiss">
        ...
        <div class="sidebar-header">
            ...
            <?php $perfil_actual='18'; include ('../menus_dinamicos/perfiles_dinamicos_eventos.php'); ?>
      ...
      <div class="table-responsive">
        <table class="table" id="niveles_table">
          <thead>
            ...
            <th class="text-center"><b>Procesados</b></th>
            <th><b>Estatus</b></th>
            <th><b>Tipo de Evento</b></th>
            <th><b>Familia Anfitriona</b></th>
            <th><b>Acciones</b></th>
          </thead>
          <tbody class="searchable" style="overflow: auto; max-height: 500px;">
            <?php while ( $dato = mysqli_fetch_assoc ( $datos ) )
            {
              ...
              if ($show){
                ...
                $sql ="SELECT COUNT(*) FROM Ventana_permisos_alumnos  WHERE id_permiso ='$id_permiso'";
                $query = mysqli_query($conexion, $sql) ;
                if ($r = mysqli_fetch_array($query) ){
                  $alumnos_totales = $r[0];
                }
                //saber cuantos alumnos hay procesados
                $sql ="SELECT COUNT(*) FROM Ventana_permisos_alumnos  WHERE id_permiso ='$id_permiso' and estatus >'1'";
                $query = mysqli_query($conexion, $sql) ;
                if ($r = mysqli_fetch_array($query) ){
                ...
                <tr class="fila_alumnos" id="fila_<?=$id_permiso?>"  style="background:<?=$color?>; border-bottom:  1px solid <?=$borde?>;"  data-row="<?php echo $dato['id_permiso']?>">
                  <td style=""><?php echo $codigo_invitacion?></td>
...
<script type="text/javascript" src="js/Peventos.js"></script>
