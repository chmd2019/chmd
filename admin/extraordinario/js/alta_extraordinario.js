...
function valida_hora_mayor(id){
  var hora_s =  $("#hora_salida_"+id).val();
  var hora_r =  $("#hora_regreso_"+id).val();
  // Date and time method
  if (hora_s >= hora_r){
    alert('Debe seleccionar un horario de regreso mayor que el horario de entrada!');
     $('#hora_regreso_'+id).timepicker('remove');
     $('#hora_regreso_'+id).val("");
  }
}

function reinicia_hora_regreso(id) {
   $('#hora_regreso_'+id).val("");
}
...
function Cancelar()
{
     window.location.replace("../extraordinario/PextraordinarioKinder.php");
}
